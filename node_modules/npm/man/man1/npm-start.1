.TH "NPM\-START" "1" "February 2018" "" ""
.SH "NAME"
\fBnpm-start\fR \- Start a package
.SH SYNOPSIS
.P
.RS 2
.nf
npm start [\-\- <args>]
.fi
.RE
.SH DESCRIPTION
.P
This runs an arbitrary command specified in the package's \fB\fP property of
its \fB\fP object\. If no \fB\fP property is specified on the
\fB\fP object, it will run \fB\fP\|\.
.P
As of ` \fIhttp://blog\.npmjs\.org/post/98131109725/npm\-2\-0\-0\fR, you can
use custom arguments when executing scripts\. Refer to npm help run\-script for
more details\.
.SH SEE ALSO
.RS 0
.IP \(bu 2
npm help run\-script
.IP \(bu 2
npm help 7 scripts
.IP \(bu 2
npm help test
.IP \(bu 2
npm help restart
.IP \(bu 2
npm help stop

.RE

